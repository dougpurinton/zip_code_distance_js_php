...
// Store XML file in global _SESSION variable. Loading time is (hopefully) faster and XML file only needs to exist when page first loads.
if (!isset($_SESSION['XML_file_data']))
{
	...
	if (file_exists($XML_file_name))
	{
	 ...
	 //set
	 $_SESSION['XML_file_data'] = $XML_file_data->asXML();
...
{
	if (isset($_SESSION['XML_file_exists'])) // not technically necessary, but it makes script even safer from undefined warnings.
	{
		if ($_SESSION['XML_file_exists'] === true)
		{
		 //get
		 $XML_file_data = new SimpleXMLElement($_SESSION['XML_file_data']);
		}
	}
	else
	{$_SESSION['XML_file_exists'] = false;}
...
else
{
 ...
 echo "Failed to open $XML_file_name.";
}
